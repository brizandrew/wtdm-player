module.exports = {
	...
	init: function (argument) {
		...
		// buttons
		this.playbackSVG.addEventListener('click',function(){
			self.togglePlayback();
		});
		this.shuttleBackSVG.addEventListener('click', function(){
			self.currentTime(self.audio.currentTime - 15);
		});
		this.shuttleForwardSVG.addEventListener('click', function(){
			self.currentTime(self.audio.currentTime + 15);
		});
	...
	createDOM: function(){
		...
		this.shuttleBackSVG = this.newElement({
			name: 'svg',
			innerHTML: this._icons.shuttleBack,
			appendTo: this.shuttleBack,
			attrs: {
				viewBox: '0 0 97.73 105.75'
			}
		});
		...
		this.shuttleForwardSVG = this.newElement({
			name: 'svg',
			innerHTML: this._icons.shuttleForward,
			appendTo: this.shuttleForward,
			attrs: {
				viewBox: '0 0 97.73 105.75'
			}
		});
	...
	load: function(data){
		...
		// clear previous interval to prevent display of NaN for duration
		clearInterval(this.loop);

		// create function to run async when duration is loaded
		...
		// load audio file and activate loop when duration is loaded
